...
describe('UserRepositoryPostgres', () => {
  ...
  describe('verifyAvailableId function', () => {
    it('should throw NotFoundError when user with id not exist', async () => {
      // Arrange
      await UsersTableTestHelper.addUser({});
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action & Assert
      await expect(userRepositoryPostgres.verifyAvailableId('user-345')).rejects.toThrowError(NotFoundError);
    });
    it('should not throw NotFoundError when user with id exist', async () => {
      // Arrange
      await UsersTableTestHelper.addUser({});
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action & Assert
      await expect(userRepositoryPostgres.verifyAvailableId('user-123')).resolves.not.toThrowError(NotFoundError);
    });
  });
